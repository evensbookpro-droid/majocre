...
interface AppSettings {
  ...
  bank_info: string;
  footer_text: string;
  blocking_thresholds: string;
  registration_form_config?: string | null;
}
...
export const AppSettingsProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const fetchSettings = async () => {
    ...
    try {
      ...
      // Try selecting all columns first using limit(1) to avoid PGRST116 on empty tables
      try {
        const { data: rows, error: selectError } = await supabase
          .from('app_settings')
          .select('id, site_name, logo_url, primary_color, secondary_color, accent_color, contact_email, contact_phone, bank_info, footer_text, blocking_thresholds, registration_form_config')
          .limit(1);
      ...
      // If that failed (e.g. due to missing columns or cache mismatch), query only the core settings
      if (error) {
        const isTableMissing = error.code === '42P01' || (error.message && error.message.includes('relation "public.app_settings" does not exist'));
        if (!isTableMissing) {
            ...
            if (!coreError) {
              if (coreRows && coreRows.length > 0) {
                data = {
                  ...
                  blocking_thresholds: localStorage.getItem('fallback_blocking_thresholds') || defaultSettings.blocking_thresholds,
                  registration_form_config: localStorage.getItem('fallback_registration_form_config') || defaultSettings.registration_form_config,
                };
              } else {
                data = null; // Table exists but is empty
              }
              error = null; // Clear error since fallback was successful (either empty table or core settings fetched)
            } else {
              error = coreError;
            }
          } catch (coreErr) {
      ...
      if (error) {
        const isTableMissing = error.code === '42P01' || (error.message && error.message.includes('relation "public.app_settings" does not exist'));
        if (isTableMissing) {
          console.warn('The "app_settings" table does not exist yet. Please run the SQL migration in Supabase. Using default local settings.');
        } else if (error.code !== 'PGRST116') {
          console.warn('Failed to fetch app settings from database, using cached/default settings:', error);
        }
      }
      ...
      if (data) {
        ...
        const newSettings = {
          ...
          blocking_thresholds: data.blocking_thresholds || localStorage.getItem('fallback_blocking_thresholds') || defaultSettings.blocking_thresholds,
          registration_form_config: data.registration_form_config || localStorage.getItem('fallback_registration_form_config') || defaultSettings.registration_form_config,
        };
  ...
  useEffect(() => {
    ...
    // Refresh app settings periodically (every 15 seconds) to ensure changes are synced
